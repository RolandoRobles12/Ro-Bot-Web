...
/**
 * Stage category for visual grouping and metrics calculation.
 * - 'new': Recently created (e.g., "Nuevo", "Solicitud")
 * - 'in_progress': Being worked on (e.g., "En proceso", "Contactado")
 * - 'advanced': Progressed significantly (e.g., "Propuesta enviada", "Negociación")
 * - 'won': Successfully closed (e.g., "Cerrado ganado", "Formalizado")
 * - 'lost': Lost or cancelled (e.g., "Cerrado perdido", "Cancelado")
 */
export type StageCategory = 'new' | 'in_progress' | 'advanced' | 'won' | 'lost';
...
/**
 * A HubSpot pipeline with user-friendly configuration.
 * Allows multiple pipelines for different products, channels, or areas.
 */
export interface Pipeline {
  id: string;
  workspaceId: string;
  name: string;                   // User-friendly name (e.g., "Ventas Kioscos")
  description?: string;
  hubspotPipelineId: string;      // The actual HubSpot pipeline ID
  icon?: string;                  // Emoji or icon name
  color?: string;                 // Brand color for this pipeline
  stages: PipelineStage[];        // Configured stages with categories
...
/**
 * Type of data source.
 * - 'pipeline': Metrics from a HubSpot pipeline
 * - 'property': Specific HubSpot properties
 * - 'google_sheets': Data from Google Sheets
 ...
 * These become available as {{variable_key}} in message templates.
 */
export interface DataSourceVariable {
  key: string;                    // Variable key (e.g., "deals_count", "amount_total")
  label: string;                  // User-friendly label (e.g., "Total de negocios")
  description?: string;           // Help text
  type: 'number' | 'currency' | 'percentage' | 'text' | 'date';
  format?: string;                // Format pattern (e.g., "$#,##0" for currency)
}

/**
 * Date range configuration for data sources.
 */
...
export interface DataSource {
  id: string;
  workspaceId: string;
  name: string;                   // User-friendly name (e.g., "Métricas Semanales Kioscos")
  description?: string;
  type: DataSourceType;
  icon?: string;                  // Emoji or icon name
  ...
  customDateStart?: Timestamp;
  customDateEnd?: Timestamp;

  // Variables this data source provides
...
/**
 * Configuration for which data sources a campaign uses.
 * Replaces the old hardcoded CampaignDataConfig.
 */
export interface CampaignDataSourceConfig {
  dataSourceIds: string[];        // IDs of DataSource entities to use
  // Optional overrides per data source
  overrides?: {
    dataSourceId: string;
    dateRange?: DateRangeType;    // Override the data source's default date range
  }[];
}
